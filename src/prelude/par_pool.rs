//! Eager parallel fold as a Lift.
//!
//! Phase 1 (sequential, via provided Exec): extracts heaps from nodes
//! into a tree of EagerNodes — the fold's accumulate establishes the
//! parent-child structure naturally.
//!
//! Phase 2 (parallel, in unwrap): recursive fork-join on the heap tree.
//! Each parent submits its children to a WorkPool, executes one child
//! in-place, and helps process queue items while waiting for the rest.
...
/// Use `WorkPool::with` for scoped lifecycle management.
pub struct WorkPool {
...
impl WorkPool {
    /// Create a pool with `n_workers` threads, run `f` with access to it,
    /// then shut down. The pool cannot escape the closure.
    pub fn with<R>(n_workers: usize, f: impl FnOnce(&Arc<Self>) -> R) -> R {
...
// ── Phase 1 types (built by the Lift) ──────────────────────
...
/// Eager parallel fold backed by a WorkPool.
pub fn par_eager<N, H, R>(pool: &Arc<WorkPool>) -> Lift<N, H, R, N, EagerNode<H>, EagerHandle<H, R>>
where
    N: Clone + 'static,
