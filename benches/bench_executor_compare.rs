//! Focused benchmark: Pool vs FusedParallel vs Rayon (Shared domain).
...
fn bench_executor_compare(c: &mut Criterion) {
    ...
    for def in scenario::all_scenarios(Scale::from_env()) {
        ...
        WorkPool::with(WorkPoolSpec::threads(3), |pool| {
            ...
            let fpar_exec = FusedParallelIn::<hylic::domain::Shared>::new(pool, FusedParallelSpec::default_for(3));
            ...
            // FusedParallel
            group.bench_with_input(
                BenchmarkId::new("hylic.fused_par.shared", &s.name),
                &(), |b, _| b.iter(|| black_box(fpar_exec.run(&s.fold, &s.treeish, &s.root))),
            );

            // Hand-rolled rayon baseline
            group.bench_with_input(
                BenchmarkId::new("hand.rayon", &s.name),
                &(), |b, _| b.iter(|| black_box(handrolled_rayon(&s))),
            );

            // Hand-rolled pool baseline
